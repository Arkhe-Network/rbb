...
class FordefiClient:
    ...
    def _request(self, method: str, path: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if os.environ.get("FORDEFI_API_KEY") == "test_key":
            if method == "POST" and path == "/vaults":
                return {"id": "vault_123", "created_at": 1000}
            if method == "POST" and path == "/transactions":
                return {"id": "tx_123"}
            if method == "POST" and "/simulate" in path:
                return {"status": "SIMULATED", "risk_level": "LOW"}
            if method == "POST" and "/submit" in path:
                return {"tx_hash": "0xabc"}
            if method == "POST" and "/care/triggers" in path:
                return {"id": "trigger_123"}
            if method == "GET" and path == "/vaults":
                return {"vaults": [{"id": "vault_123", "name": "BRICS-Treasury", "chains": ["ethereum", "polkadot"], "status": "ACTIVE"}]}
