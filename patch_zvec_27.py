...
for i, line in enumerate(lines):
    if "            \"action\": [float(action[0])] if hasattr(action, '__len__') else [float(action)]\n" in line:
        new_lines.append("            \"action\": [float(action[0])] if hasattr(action, '__len__') and len(action) > 0 else [0.0] if hasattr(action, '__len__') and len(action) == 0 else [float(action)]\n")
        continue
