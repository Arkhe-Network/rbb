...
def train_model(model_id, dataset_id, lora_r, lora_alpha, lr, batch_size, quantization, progress=gr.Progress()):
    ...
    try:
        ...
        def formatting_func(examples):
            ...
            # fallback for alpaca-like datasets
            elif "instruction" in examples and "output" in examples:
                outputs = []
                for inst, inp, out in zip(examples.get("instruction", []), examples.get("input", [""] * len(examples.get("instruction", []))), examples.get("output", [])):
                    if inp:
                ...
                return {"text": [str(examples)] * len(examples[list(examples.keys())[0]])}
