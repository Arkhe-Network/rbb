...
class FeaturizerAgent(Agent):
    """Agente 1: Transforma estruturas em números e otimiza memória."""
    def __init__(self, name: str = "Featurizer", output_path: str = "features_output.parquet"):
        super().__init__(name)
    ...
    def execute(self, state: Dict) -> Dict:
        ...
        # Salva em parquet para eficiência em datasets massivos
        df = pd.DataFrame(features)
        df.columns = [str(c) for c in df.columns] # parquet requires string column names
        df.to_parquet(self.output_path)
        logger.info(f"Features (float32) salvas em {self.output_path}")
...
class TopoMAS:
    def __init__(self, features_path: str = "features_output.parquet"):
        self.kg = KnowledgeGraph()
        self.agents = [
            FeaturizerAgent("Featurizer", output_path=features_path),
