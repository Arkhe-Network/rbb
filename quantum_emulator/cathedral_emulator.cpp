...
constexpr size_t SIG_SIZE = 3952;                     // Real SPHINCS+ size
...
public:
    ...
    void sign(const uint8_t* msg, size_t msg_len, uint8_t* sig) const {
        sphincs_sign(msg, msg_len, sk, sig);
    }

    bool verify(const uint8_t* msg, size_t msg_len, const uint8_t* sig) const {
        return sphincs_verify(msg, msg_len, sig, pk);
    }
};
...
public:
    ...
    void advance_tick(uint64_t skip) { tick += skip; }
