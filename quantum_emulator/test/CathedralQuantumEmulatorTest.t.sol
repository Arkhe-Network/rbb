...
import "../src/CathedralSPHINCSVerifier.sol";
...
contract CathedralQuantumEmulatorTest is Test {
    ...
    function testAttackFastForward() public {
        // First we need a valid tick to set latestTick > 0
        vm.prank(emulatorAddress);
    ...
    function testAttackReplay() public {
        uint64 tickId = 100;
        bytes32 oldBlockHash = bytes32(uint256(1));
        bytes32 newBlockHash = bytes32(uint256(2));
        ...
        // Tenta reapresentar com hash novo mas mesma assinatura
        vm.prank(emulatorAddress);
        vm.expectRevert("Tick already passed");
