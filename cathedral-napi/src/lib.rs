...
impl CathedralAgent {
    ...
    pub async fn tick(&self) -> napi::Result<String> {
        // We can't spawn a future that holds a standard MutexGuard across an await point.
        // Instead, we just spawn a blocking task since tick_zk_with_accelerator is an async function
        // that completes the tick. However, we'll need to wrap it inside a tokio block.
        // Actually, let's just make the lock scoped if we can, or just not spawn it and await here
        // since we are in an async function.
        // Wait, the napi-rs async macro creates a promise.

        let core_arc = self.core.clone();

        // Spawn a blocking task or use block_in_place to avoid holding a lock across await
        let result = self.runtime.spawn_blocking(move || {
            let mut core = core_arc.lock().unwrap();
            // Since tick is async, we need a runtime to block on it,
            // or better, if we can change tick to be synchronous or just run it synchronously.
    ...
    pub async fn get_recommendations(
        ...
        let recs = self.runtime.spawn_blocking(move || {
            let core = core_arc.lock().map_err(|e| e.to_string())?;
            let rt = tokio::runtime::Handle::current();
            rt.block_on(async {
                core.get_picoads_recommendations(&query, hub.as_deref(), max_results).await
            })
        }).await
        .map_err(|e| napi::Error::from_reason(e.to_string()))?
