...
def causal_without_mechanism(claim: str) -> bool:
    low = claim.lower()
    has_causal = any(re.search(rf"\b{c}\b", low) for c in CAUSAL_CONNECTORS)
    has_mechanism = any(re.search(rf"\b{m}\b", low) for m in MECHANISM_MARKERS)
    return has_causal and not has_mechanism


def jargon_terms(claim: str) -> list[str]:
    found = set()
    # List of common capitalized emphasis or instructional words to ignore
    ignored_words = {
        "P", "ECHO", "LEMMA", "THEOREM", "CONJECTURE",
        "NUNCA", "EXATAMENTE", "DEVE", "PRESENÇA", "PRESENÇA VS",
        "FORMALIZE", "DECOMPOSE", "VERIFY", "ATTACK", "LABEL", "FOL"
    }
    for m in JARGON_CANDIDATES.finditer(claim):
        term = m.group(0).strip()
        if len(term) > 1 and term.upper() not in ignored_words:
            found.add(term)
    return sorted(found)
