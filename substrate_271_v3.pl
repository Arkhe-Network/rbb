...
similarity_semantic(Text1, Text2, Score) :-
    ...
    ( catch(http_post('http://localhost:8002/similarity', json(JSON), Reply, [timeout(30)]), _, fail) ->
...
compare_theories(Theory1, Theory2, Report) :-
    ...
    ; theory(Theory1, O1, UV1, S1, C1, H1, _),
      ...
      Overall is (OntologySim + UVSim + SymSim + CosmoSim + HoloSim) / 5.0,
...
compare_dimension(Theory1, Theory2, Dimension, Score) :-
    compare_theories(Theory1, Theory2, Report),
    ( Dimension = ontology -> get_dict(ontology_similarity, Report, Score)
    ; Dimension = uv_ir -> get_dict(uv_similarity, Report, Score)
    ; Dimension = symmetries -> get_dict(symmetry_similarity, Report, Score)
    ; Dimension = cosmology -> get_dict(cosmology_similarity, Report, Score)
    ; Dimension = holography -> get_dict(holography_similarity, Report, Score)
    ; Score = 0.5 ).

similarity_score(Theory1, Theory2, Score) :-
    compare_theories(Theory1, Theory2, Report),
...
hubble_tension_test(Theory, Score) :-
    ...
    random(R_float), Score is Base * 0.9 + 0.1 * R_float.
...
physlean_integration(GUTID, Status) :-
    ( current_predicate(physlean:generate_lean_theorem_physlean/2), physlean:generate_lean_theorem_physlean(GUTID, _LeanCode) ->
...
generate_heatmap(Format, _Data) :-
    all_theories_comparison(_Matrix),
...
rsi_compare_theories(Theory1, Theory2, Score) :-
    ( current_predicate(rsi_state/1), rsi_state(State) ->
