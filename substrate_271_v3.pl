...
% C1: Para rodar sem dependência do python http. Substitui chamada HTTP.
...
physlean_integration(_GUTID, Status) :-
...
rsi_compare_theories(Theory1, Theory2, Score) :-
    % Mock the RSI state for the test script
    Generation = 1,
    compare_theories(Theory1, Theory2, Report),
...
guts_gravity_coupling(GUTID, Coupling) :-
    % Mock the guts DB lookup for the test
    random_float(R),
...
wormgraph_comparison(ComparisonID, Data) :-
    ...
    Block = _{ event: 'gravity_comparison',
               ...
               data: Data,
               timestamp: Timestamp },
    format('[WORMGRAPH] Bloco inserido: ~w~n', [Block]).
