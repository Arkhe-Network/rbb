...
class CathedralAgent:
    ...
    def _register_default_tools(self):
        ...
        class CoreTickToolInternal(BaseTool):
            name = "core_tick"
    ...
    async def run(self, user_input: str) -> Dict[str, Any]:
        ...
        iteration = 0
        final_answer = None
        current_prompt = self.planner._build_initial_prompt(user_input, context, self.tools)
        steps = []

        while iteration < self.planner.max_iterations and final_answer is None:
            # Plan next step using planner
            try:
                thought, action, action_input = await self.planner._get_thought_and_action(current_prompt)
            except Exception as e:
            ...
            if action == "FINISH":
                ...
                step = ReActStep(
                    ...
                    observation=None, step_index=iteration, timestamp=time.time()
                )
                steps.append(step)
                break
            ...
            if not safety.allowed:
                ...
                step = ReActStep(
                    thought=thought, action=action, action_input=action_input,
                    observation=observation, step_index=iteration, timestamp=time.time()
                )
                steps.append(step)
                await self.memory.add(step, {"error": safety.reason})
                final_answer = f"Safety block: cannot perform {action}. {safety.reason}"
                break
            ...
            # Ancoragem
            if self.planner.anchor:
                import time
                from agent.planning.react_planner import TemporalChainAnchor
                from dataclasses import asdict
                step.timestamp = time.time()
                step.block_hash = TemporalChainAnchor.anchor_step(asdict(step))

            steps.append(step)
            await self.memory.add(step, getattr(locals().get('result'), 'data', observation))
