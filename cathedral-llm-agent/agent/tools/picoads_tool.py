...
class PicoAdsTool(BaseTool):
    ...
    def description(self) -> str:
        return "Obtém recomendações personalizadas de anúncios/produtos baseadas no contexto do usuário."
    @property
    def name(self) -> str:
        return "picoads"
    ...
    def __init__(self):
        # Configuração
        self.use_kernel_driver: bool = True   # se False, usa HTTP API
        self.pipe_name: str = r"\\.\pipe\PicoAdsService"
        self.api_base: str = os.getenv("PICOADS_API_BASE", "https://api.picoads.cathedral.io/v1")
        self.api_key: str = os.getenv("PICOADS_API_KEY", "")
        self.trust_tier: int = 200            # 0-255, 200+ requer prova de memória
        self.memory_proof_required: bool = True
    ...
    async def run(self, **kwargs) -> ToolResult:
        ...
        if "error" in result:
            return ToolResult(success=False, error=result["error"], data=result.get("recommendations", []))
    ...
    async def _call_kernel_driver(self, ctx_hash: str, max_results: int) -> Dict:
        ...
        return {"recommendations": [{"id": "ad1", "name": "Quantum Computer"}], "status": "success (stub)"}
