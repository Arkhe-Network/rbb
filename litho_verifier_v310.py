...
def extract_all_parameters(text: str) -> List[Tuple[str, float, str]]:
    ...
    pattern = re.compile(
        r'(?P<name>\b[a-zA-Z]\w*)\s*[:=<>≤≥]\s*(?P<value>[+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?)\s*(?P<unit>[a-zA-Zµμ/³²0-9]+)?',
        re.IGNORECASE
    )
...
def run(path: Path, equipment_file: Optional[Path] = None, tolerance_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    for idx, claim in enumerate(claims):
        ...
        used_equipment = equipment or current_equipment
        params = extract_all_parameters(claim)

        if not used_equipment:
            for p, v, u in params:
        ...
        profile = EquipmentProfile.from_dict(used_equipment, equipment_db[used_equipment])
