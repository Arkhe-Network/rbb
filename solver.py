...
def load_mesh_from_gmsh(filename, refine_near_axis=True, axis_R=1.4, axis_Z=0.0, refinement_factor=0.5):
    ...
    ret = gmsh.read_from_msh(filename, MPI.COMM_WORLD, 0, gdim=2)
    domain = ret[0]
    cell_markers = ret[1]
    facet_markers = ret[2]
...
class GradShafranovSolverAMG:
    ...
    def __init__(self, mesh_file, R0=1.4, a=0.9, B0=12.0, use_amg=True, refine_axis=True):
        ...
        # Convert to numpy.float64 first, then to default_scalar_type, this avoids issues with UFL constants initialized directly with np.float64
        self.psi_max = fem.Constant(self.mesh, default_scalar_type(1.0))
        self.psi_min = fem.Constant(self.mesh, default_scalar_type(0.0))
    ...
    def _setup_snes(self):
        ...
        # Tolerâncias
        self.snes.setTolerances(rtol=1e-6, atol=1e-8, stol=1e-10, max_it=50)
        ...
        if self.use_amg:
            ...
            # petsc4py may not expose setHYPRECoarsenType/setHYPRENodalType directly
            # You can set them via options instead
            opts = PETSc.Options()
            opts['pc_hypre_boomeramg_coarsen_type'] = 'HMIS'
            opts['pc_hypre_boomeramg_nodal_coarsen'] = 1
    ...
    # --------------------------------------------------------------------
    class GradShafranovProblem:
        def __init__(self, solver, psi):
            self.solver_obj = solver
            self.psi = psi
            self.bcs = solver.bcs

            # Setup the forms correctly for NonlinearProblem
            import dolfinx.fem as fem
            self.F_form = fem.form(self.solver_obj.residual_form(self.psi))
            self.J_form = fem.form(self.solver_obj.jacobian_form(self.psi, TrialFunction(self.solver_obj.V)))
            # Older dolfinx might need petsc_options_prefix to be omitted if not forced, but here it forces it as kwargs. Let's provide an empty string or omit it using positional if possible, but the error said "keyword-only argument". But dolfinx version 0.10 seems to require it or it's a bug in signature. Let's just avoid NonlinearProblem inheritance and use basic object for PETSc SNES.
            pass

        def F(self, snes, x, b):
            x.copy(self.psi.x.petsc_vec)
            self.psi.x.scatter_forward()
            with b.localForm() as b_local:
                b_local.set(0.0)
            fem.petsc.assemble_vector(b, self.F_form)
            fem.petsc.apply_lifting(b, [self.J_form], bcs=[self.bcs])
        ...
        def J(self, snes, x, A, P):
            ...
            self.psi.x.scatter_forward()
            A.zeroEntries()
            fem.petsc.assemble_matrix(A, self.J_form, bcs=self.bcs)
            A.assemble()
    ...
    def solve(self, psi_init=None, max_picard=5, aitken=True):
        ...
        if psi_init is not None:
            ...
            R0, a = self.R0, self.a
            def initial_guess(x):
                r2 = ((x[0] - R0)/a)**2 + (x[1]/a)**2
                val = 0.5 * (1 - r2)
                return np.maximum(val, 0.0)
            self.psi.interpolate(initial_guess)
        ...
        for picard_iter in range(max_picard):
            ...
            import dolfinx.fem as fem
            # Create vectors and matrices for SNES
            b = petsc_fem.create_vector(self.V)
            J_mat = petsc_fem.create_matrix(problem.J_form)

            self.snes.setFunction(problem.F, b)
            self.snes.setJacobian(problem.J, J_mat, J_mat)

            # Resolve
            print(f"\n  Picard iter {picard_iter+1}/{max_picard}")
            self.snes.solve(None, self.psi.x.petsc_vec)

            converged = self.snes.getConvergedReason() > 0
            n_its = self.snes.getIterationNumber()
            print(f"  SNES convergence reason: {self.snes.getConvergedReason()}, iterations: {n_its}")
...
if __name__ == "__main__":
    ...
    with io.VTXWriter(MPI.COMM_WORLD, "psi_solution.bp", [gs.psi]) as vtx:
        vtx.write(0.0)
    print("\n  Solução salva em psi_solution.bp")
