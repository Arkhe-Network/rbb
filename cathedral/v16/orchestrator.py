...
class CathedralOrchestrator:
    ...
    async def propose_action(self, deterministic: bool = False) -> ActionProposal:
        ...
        action = self.rl_agent.select_action(features.squeeze(0).detach().cpu().numpy(), deterministic)
    ...
    async def validate_safety(self, proposal: ActionProposal,
                              agent_id: str = "cathedral_agent", qpos: Any = None) -> ActionProposal:
        """
        ...
        # Validação Z3
        if qpos is not None:
            is_safe, violations = self.ontology.validate_with_explanation(qpos, target_id, force)
            proposal.symbolic_violations = violations
        else:
    ...
    async def learn_from_experience(self, reward: float, next_image: Optional[np.ndarray] = None):
        ...
        # Embedding para HNSW
        embedding = self.current_rssm_state.get_features().squeeze(0).detach().cpu().tolist()

        # Próximo estado (se disponível)
        if next_image is not None:
            next_frame = await self.perceive(next_image)
            next_state = self.current_rssm_state.get_features().squeeze(0).detach().cpu().numpy()
        else:
            next_state = self.current_rssm_state.get_features().squeeze(0).detach().cpu().numpy()

        # Armazena transição
        current_state = self.current_rssm_state.get_features().squeeze(0).detach().cpu().numpy()
        # Recupera última ação proposta
        # (Em loop real, a ação executada seria armazenada)
        action = torch.zeros(self.action_dim).detach().numpy()  # Placeholder
    ...
    async def run_cycle(self, raw_image: np.ndarray, reward: float = 0.0, qpos: Any = None) -> Dict:
        """
        ...
        proposal = await self.validate_safety(proposal, qpos=qpos)
        ...
        return {
            ...
            "violations": proposal.symbolic_violations
        }
