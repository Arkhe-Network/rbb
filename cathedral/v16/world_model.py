...
class WorldModelRSSM(nn.Module):
    ...
    def imagine_rollout(self, start_state: RSSMState, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # return torch.stack(features_list), torch.stack(rewards_list) # Dreamer needs 3 results as expected by orchestrator? Let's check original. Original imagine_rollout:
        # returns features, rewards, continues. Continues is not in patched version. We need continues or orchestrator will break.
        # Let's add continues to patched version to avoid orchestrator breakdown.
        return torch.stack(features_list), torch.stack(rewards_list), torch.ones_like(torch.stack(rewards_list))
