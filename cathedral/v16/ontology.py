...
class SymbolicSafetyEngine:
    ...
    def validate_with_explanation(self, qpos: Any, target_id: str, force: float) -> Tuple[bool, List[str]]:
        """
        Validação aprimorada com qpos real do MuJoCo.
        """
        violations = []
        is_safe = self.validate_action_safety("cathedral_agent", "proposed_action", target_id, force)

        if not is_safe:
            # Explica com qpos
            violations.append(f"force={force:.2f} exceeds safe threshold for {target_id}")
