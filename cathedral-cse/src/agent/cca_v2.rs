...
use crate::tools::SessionManager;
...
impl CCAgentV2 {
    pub async fn new(
        ...
        let tokenizer = Tokenizer::from_pretrained("meta-llama/Llama-3.1-8B", None)
        ...
        let sahoo = Arc::new(SahooPlus::new());
    ...
    pub async fn process(&mut self, user_input: &str, session_id: &str) -> Result<String, String> {
        ...
        ctx.history = session.history.clone();
