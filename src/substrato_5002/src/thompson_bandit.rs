...
impl ThompsonBandit {
    ...
    pub fn record_reward(&mut self, pattern_id: &str, performance: &PatternPerformance) {
        ...
        let dist = self.distributions.get_mut(pattern_id).expect("Pattern must be registered before recording reward");
...
mod tests {
    ...
    fn test_thompson_selection_convergence() {
        ...
        // Note: With mocked Beta it might not consistently converge depending on the RNG values if they are pure uniform, but the update logic works.
