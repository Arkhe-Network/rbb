...
// use rand::distributions::{Distribution, Beta}; // Note: need rand package. We will implement simple or add dependency.
...
impl BetaDistribution {
    ...
    pub fn sample(&self) -> f64 {
        // Emulando, ja que pode não ter statrs ou rand_distr no toml
        let mut sum = 0.0;
        for _ in 0..10 { sum += rand::random::<f64>(); }
        let normal = (sum - 5.0) / (10.0 / 12.0_f64).sqrt(); // Central limit approx
        let mean = self.mean();
        let std = self.variance().sqrt();
        (mean + normal * std).clamp(0.0, 1.0)
    }
...
mod tests {
    ...
    fn test_thompson_selection_convergence() {
        ...
        assert!(ranked[0].1 > 0.0, "'cot' mean should be > 0.0");
