...
pub mod zvec_proto {
    ...
    // Fake proto
    pub struct ZvecClient<T> { _t: std::marker::PhantomData<T> }
    impl<T> ZvecClient<T> {
        pub fn new(_t: T) -> Self { Self { _t: std::marker::PhantomData } }
        pub async fn generate_embedding(&mut self, _req: tonic::Request<EmbeddingRequest>) -> Result<tonic::Response<EmbeddingResponse>, tonic::Status> {
            Ok(tonic::Response::new(EmbeddingResponse { embedding: vec![] }))
        }
        pub async fn search(&mut self, _req: tonic::Request<SearchRequest>) -> Result<tonic::Response<SearchResponse>, tonic::Status> {
            Ok(tonic::Response::new(SearchResponse { results: vec![] }))
        }
        pub async fn index(&mut self, _req: tonic::Request<IndexRequest>) -> Result<tonic::Response<IndexResponse>, tonic::Status> {
            Ok(tonic::Response::new(IndexResponse {}))
        }
        pub async fn hybrid_search(&mut self, _req: tonic::Request<SearchRequest>) -> Result<tonic::Response<SearchResponse>, tonic::Status> {
            Ok(tonic::Response::new(SearchResponse { results: vec![] }))
        }
    }
    pub struct EmbeddingRequest { pub text: String, pub model: String, pub dimensions: u32 }
    pub struct EmbeddingResponse { pub embedding: Vec<f32> }
    pub struct SearchRequest { pub query_vector: Option<Vector>, pub top_k: u32, pub filter: Option<String>, pub use_hnsw: bool, pub use_bm25: bool, pub rrf_weight: f64 }
    pub struct SearchResponse { pub results: Vec<MemoryEntryResult> }
    pub struct IndexRequest { pub entry: Option<MemoryEntry>, pub batch: bool }
    pub struct IndexResponse { }
    #[derive(Clone)]
    pub struct MemoryEntry { pub id: String, pub content: String, pub embedding: Option<Vector>, pub metadata: String, pub timestamp: u64 }
    pub struct MemoryEntryResult { pub entry_id: String, pub score: f64, pub content: String, pub metadata: String }
    #[derive(Clone)]
...
impl ZvecBridge {
    ...
    pub async fn semantic_search(
        ...
        let request = Request::new(SearchRequest {
            ...
            top_k: top_k as u32,
            filter: filter.map(|f| serde_json::to_string(&f).unwrap()),
