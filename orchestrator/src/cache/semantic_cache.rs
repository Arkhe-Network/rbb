...
use qdrant_client::qdrant::{
    self, CreateCollection, PointStruct, SearchPoints, VectorParams, Vectors, VectorsConfig
};
...
impl SemanticCache {
    ...
    pub async fn set(&self, prompt: &str, response: &str) -> Result<()> {
        ...
        let point = PointStruct::new(
            ...
            embedding,
            HashMap::from([
                ("prompt".to_string(), serde_json::Value::String(prompt.to_string()).into()),
                ("response".to_string(), serde_json::Value::String(response.to_string()).into()),
            ]),
        );
        self.client
