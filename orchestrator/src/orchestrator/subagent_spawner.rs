...
impl SubagentSpawner {
    ...
    pub async fn spawn(
        ...
        let parent_policies: Vec<crate::attestation::PolicyDescriptor> = match self.policy_engine.list_active_policies().await { Ok(v) => v, Err(_) => vec![] };
    ...
    pub async fn terminate(&self, id: &str) -> Result<Subagent, String> {
        ...
        if let Some(idx) = pos {
            ...
            let _ = subagent.deactivate().await;
...
impl std::fmt::Debug for dyn crate::attestation::AttestationSigner + Send + Sync {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "AttestationSigner")
    }
}
