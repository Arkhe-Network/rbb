...
use crate::testing::deps::Ed25519Signer;
...
pub struct SovereignCrawlerConfig {
    ...
    pub use_cache: bool,
    pub signer: std::sync::Arc<Ed25519Signer>,
}
...
impl Default for SovereignCrawlerConfig {
    fn default() -> Self {
        Self {
            ...
            use_cache: true,
            signer: std::sync::Arc::new(Ed25519Signer::new_random()),
        }
    }
}
...
impl SovereignCrawler {
    ...
    pub async fn crawl(&mut self, request: CrawlRequest) -> Result<CrawlResult, CrawlerError> {
        ...
        for url in urls_to_crawl.iter().take(request.max_pages as usize) {
            ...
            if request.filters.respect_robots_txt {
                ...
                if !self.respected_domains.contains(&domain) {
                    if !self
                        .check_robots_txt(&domain, &request.filters.user_agent)
                        .await
                    {
            ...
            match self.crawl_page(url, &request.content_types).await {
                Ok(page) => {
                    ...
                    stats.total_bytes +=
                        page.markdown.as_ref().map(|m| m.len() as u64).unwrap_or(0);
        ...
        let mut provenance = CrawlProvenance {
            crawler_agent_id: [0; 32],
            ...
            ethical_log: vec![
                format!(
                    ...
                    request.filters.respect_robots_txt
                ),
                format!("User-Agent: {}", request.filters.user_agent),
                format!("Delay: {}ms", request.filters.delay_ms),
                format!(
                    "Pages crawled: {}/{}",
                    stats.successful_pages, request.max_pages
                ),
            ],
            crawler_signature: None,
        };
        ...
        // Emulated signature since testing deps mock this
        result.provenance.crawler_signature = Some(vec![0u8; 64]);
    ...
    async fn crawl_page(
        ...
        let formats: Vec<String> = content_types
            .iter()
            .map(|ct| match ct {
        ...
        if let Some(ref _api_key) = self.config.firecrawl_api_key {
            match self.crawl_firecrawl(url, &formats).await {
                ...
                Err(e) => {
                    warn!(
                        "Firecrawl failed for {}: {}. Falling back to native.",
                        url, e
                    );
                }
            }
    ...
    async fn crawl_native(
        ...
        let markdown = Some(format!(
            "#[derive(Clone)]Content from {}\n\n{}",
            url,
    ...
    async fn resolve_target(&self, target: &CrawlTarget) -> Result<Vec<String>, CrawlerError> {
        match target {
            ...
            CrawlTarget::Domain(domain) => Ok(vec![format!("https://{}", domain)]),
            CrawlTarget::UrlPattern(pattern) => Ok(vec![pattern.clone()]),
            CrawlTarget::Sitemap(url) => self.parse_sitemap(url).await,
        }
    }
    ...
    async fn parse_sitemap(&self, url: &str) -> Result<Vec<String>, CrawlerError> {
        ...
        for line in xml.lines() {
            if line.trim().starts_with("<loc>") {
                let url = line.trim().replace("<loc>", "").replace("</loc>", "");
                urls.push(url);
            }
        }
