...
use orchestrator::testing::deps::{AttestationManager, IdentityAttestation, Ed25519Signer, GeometricPolicyEngine, DummyTrajectoryStore, SubagentSpawner, SandboxType, create_sandbox, AttestationSigner};

async fn setup_test_environment() -> (
    Arc<SubagentSpawner>,
    Arc<AttestationManager>,
    Arc<DummyTrajectoryStore>,
    Arc<Ed25519Signer>,
) {
...
async fn test_performance_agent_basic() {
    ...
    let result = agent.run_test(&context).await.unwrap();
    // Pode passar ou falhar, mas deve executar sem erro
}
...
async fn test_chaos_agent_basic() {
    ...
    for i in 0..5 {
        let _ = spawner.spawn(&format!("chaos_test_{}", i), vec!["echo".to_string()]).await.unwrap();
    }
    ...
    let result = agent.run_test(&context).await.unwrap();
    assert!(result.details["successful_kills"].as_u64().unwrap_or(0) >= 0);
}
