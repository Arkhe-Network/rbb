...
class SlowBrainSGLang:
    ...
    async def reason(
        ...
        # Tenta Windows Native via fallback bridge antes se ativado
        native_result = await self._try_windows_native(messages)
        if native_result is not None:
            native_result["latency_ms"] = (time.perf_counter() - t0) * 1000
            native_result["source"] = "windows_native"
            return self._validate(native_result)

        # Tenta com retries no SGLang
