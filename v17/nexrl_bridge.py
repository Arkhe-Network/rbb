...
class InteractionLogger:
    ...
    def __init__(self, data_dir="./nexrl_data"):
    ...
    def log_interaction(self, observation, fast_action, slow_decision, reward, route):
        ...
        # Convert float32 values to standard float if necessary
        ...
        entry = {
            ...
            "fast_brain_action": [convert_to_float(x) for x in fast_action] if hasattr(fast_action, '__iter__') else list(fast_action),
            "slow_brain_decision": slow_decision,
            "reward": convert_to_float(reward),
            "route": route,  # "fast" ou "slow"
            "safety_approved": slow_decision.get("safety_override", False) if isinstance(slow_decision, dict) else False,
        }
