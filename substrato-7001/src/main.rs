...
async fn main() -> anyhow::Result<()> {
    ...
    // 1. Configuração
    let webhook_config = polar_handler::WebhookConfig::from_env()?;
    let dlq: polar_handler::DeadLetterQueue = Arc::new(RwLock::new(Vec::new()));

    // 2. Inicializa handler
    let webhook_handler = Arc::new(
        polar_handler::PolarWebhookHandler::new(webhook_config, Arc::clone(&dlq))
    );
    ...
    // 4. Router Axum
    let app = polar_handler::create_webhook_router(webhook_handler, dlq);
