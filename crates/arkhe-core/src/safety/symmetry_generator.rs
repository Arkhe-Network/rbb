...
impl SymmetryGenerator {
    ...
    pub fn preserves_manifold(&self, from: &SystemState, to: &SystemState) -> TransitionSafety {
        match (self.is_in_manifold(from), self.is_in_manifold(to)) {
            ...
            (ManifoldResult::Inside, ManifoldResult::Degraded(v)) => TransitionSafety::Degraded {
            ...
            (ManifoldResult::Inside, ManifoldResult::Outside { violation, .. }) => {
                TransitionSafety::CriticalEscape {
                    violation,
                    state: to.clone(),
                }
            }
...
impl Invariant for AgentCountInvariant {
    ...
    fn class(&self) -> InvariantClass {
        InvariantClass::High
    } // C1-fix: HIGH, não CRITICAL
    fn check(&self, state: &SystemState) -> bool {
        state.agent_count <= state.config.max_agents
    }
    fn margin(&self, state: &SystemState) -> f64 {
        if state.agent_count > state.config.max_agents {
            return 0.0;
        }
...
impl Invariant for SandboxFuelInvariant {
    ...
    fn margin(&self, state: &SystemState) -> f64 {
        ...
        if state.sandbox_fuel < state.config.min_fuel {
            return 0.0;
        }
...
impl Invariant for EntropyInvariant {
    ...
    fn margin(&self, state: &SystemState) -> f64 {
        ...
        if state.entropy_bits < state.config.min_entropy {
            return 0.0;
        }
        ((state.entropy_bits - state.config.min_entropy) as f64 / state.config.min_entropy as f64)
            .min(1.0)
    }
}
...
impl Invariant for CapabilityInvariant {
    ...
    fn margin(&self, state: &SystemState) -> f64 {
        ...
        if max_extra <= 0 {
            ...
            (extra as f64 / max_extra as f64).min(1.0).max(0.0)
        }
    }
}
