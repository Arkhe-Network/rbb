...
impl TaprootAgent {
    pub fn new(did: Did, client: Arc<tokio::sync::Mutex<TaprootClient>>, wormgraph: Arc<Wormgraph>) -> Self {
        let permissions = AgentPermissions {
            agent_did: did.clone(),
            operations: vec![],
            signature: vec![],
        };
        Self { did, client, permissions, wormgraph }
    }
}
...
impl Agent for TaprootAgent {
    ...
    async fn run(&self, prompt: &str, session_id: &str) -> Result<serde_json::Value, String> {
        self.wormgraph.append(ProvenanceEntry {
