...
impl PhononPrecursorDetector {
    ...
    fn update_precursors(&mut self, modes: &[SpectralMode]) {
        ...
        // Store SNR values for updated modes to compute confidence later
        let mut snr_for_updated = vec![None; self.active_precursors.len()];
        ...
        for mode in &candidate_modes {
            ...
            for (i, (freq_key, sig)) in self.active_precursors.iter_mut().enumerate() {
                ...
                if freq_diff <= tolerance {
                    ...
                    *freq_key = mode.frequency; // Update tracking frequency
                    ...
                    seen[i] = true;
                    snr_for_updated[i] = Some(mode.snr);
                    matched = true;
                    break;
                }
            }
            ...
            if !matched {
                ...
                // Add false to seen since this is a new entry
                seen.push(true); // Treat as seen in this window to keep it
                snr_for_updated.push(None); // Confidence already computed
        ...
        // Compute confidence for updated precursors
        for i in 0..self.active_precursors.len() {
            if let Some(snr) = snr_for_updated[i] {
                let sig = &self.active_precursors[i].1;
                // Clone sig to pass it in without holding mutable borrow
                let sig_clone = sig.clone();
                let conf = self.compute_confidence(&sig_clone, snr);
                self.active_precursors[i].1.confidence = conf;
            }
        }
    ...
    fn compute_confidence(&self, sig: &PrecursorSignature, snr: f64) -> f64 {
        ...
        (persistence_factor * 0.3 + snr_factor * 0.3 + drift_clarity.min(1.0) * 0.2 + trend_clarity.min(1.0) * 0.2)
            .min(1.0)
    }
...
mod tests {
    ...
    fn test_detect_softening_precursor() {
        ...
        assert!(event.signature.amplitude_trend > 0.0, "Expected positive trend");
        assert!(
