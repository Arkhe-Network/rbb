...
impl ModeTracker {
    ...
    pub fn process_window(&mut self, samples: &[f64]) -> Vec<SpectralMode> {
        assert_eq!(samples.len(), self.window_size, "sample count must equal window_size");
        ...
        let windowed: Vec<Complex<f64>> = samples
            ...
            .enumerate()
            .map(|(i, &s)| {
                let hann = 0.5 * (1.0 - (2.0 * std::f64::consts::PI * i as f64 / (self.window_size as f64 - 1.0)).cos());
        ...
        let floor = sorted_mags.get(sorted_mags.len() / 2).copied().unwrap_or(1e-10);
    ...
    pub fn frequency_drift(&self, target_freq: f64, window_range: usize) -> Option<f64> {
        ...
        if var_x.abs() < 1e-12 { return None; }
...
mod tests {
    ...
    fn test_detect_single_tone() {
        ...
        assert!((dominant.frequency - 100.0).abs() < 5.0,
            "Expected ~100 Hz, got {}", dominant.frequency);
        assert!(dominant.amplitude > 50.0, "Amplitude too low: {}", dominant.amplitude);
    }
