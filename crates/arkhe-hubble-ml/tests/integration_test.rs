...
use arkhe_hubble_ml::phonon::{
    ...
    activation::{ActivationPolicy, PhononInferencePipeline, InferenceState},
};
...
fn generate_sensor_scenario(fs: f64) -> Vec<f64> {
    ...
    for i in 0..total_samples {
        ...
        let (freq, amp, noise_level) = if t < 2.0 {
            ...
            let burst = (2.0 * PI * 120.0 * t).sin() * 0.5
                + (2.0 * PI * 80.0 * t).sin() * 0.3;
            signal[i] = burst;
            continue;
        };
...
fn test_full_precursor_pipeline() {
    ...
    println!("Active precursors at end: {}", pipeline.active_precursors().len());
