...
impl PlausibleReasoner {
    ...
    pub fn propose_structural_edits(
        ...
        // clone the rules to borrow later
        let similar_rules: Vec<PolicyRule> =
            similar.into_iter().map(|s| s.new_rule.clone()).collect();
        for record_rule in similar_rules {
            // Generate analogies based on the successful mutation pattern
            let mutation = self.analogical_transfer(&record_rule, current_policy);
    ...
    fn analogical_condition(&mut self, cond: &Condition) -> Condition {
        match cond {
            ...
            Condition::Or(left, right) => self.analogical_condition(left),
        }
    }
