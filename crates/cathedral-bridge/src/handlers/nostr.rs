...
impl NostrHandler {
    pub async fn handle(
        ...
        .to_event(&nostr_sdk::Keys::generate())
        .map_err(|e| Status::internal(format!("Falha na criação: {}", e)))?; // Em produção, usar chave configurada
        ...
        info!("✅ Evento publicado: {}", published.to_hex());

        Ok(Response::new(NostrPublishResponse {
            success: true,
            event_id_hex: published.to_hex(),
            relay_urls: vec![], // mock
            error: None,
