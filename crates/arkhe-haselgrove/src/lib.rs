...
impl DensityProfile for ChapmanProfile {
    ...
    fn collision_frequency(&self, r: f64, _theta: f64, _phi: f64) -> f64 {
        let h = self.altitude(r);
        let h_ref = 100_000.0;
        let scale_h = 50_000.0;
        1.0e6 * (-(h - h_ref) / scale_h).exp()
    }
}
    ...
    M: MagneticField,
{
    ...
    pub fn n2(&self, state: &RayState) -> f64 {
    ...
    pub fn dn2_dr_dtheta(&self, state: &RayState) -> (f64, f64) {
        const EPS: f64 = 1e-3;
        let eps2 = 2.0 * EPS;
    ...
    pub fn derivatives(&self, state: &RayState) -> (f64, f64, f64, f64, f64, f64) {
        const C: f64 = 299792458.0;
        let factor = C * C / (self.omega * self.omega);
        let r = state.r;
