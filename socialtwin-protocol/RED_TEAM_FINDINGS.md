...
1. Attacker deploys a contract `Sink` on RBB with a function
...
**Impact.**
- **Direct:** unauthenticated, repeatable drain of the relayer's RBB ETH (denial-of-service for all legitimate
...
- A legitimate `execute` simulates against `nonce = N`. Another relayed (or direct) tx consumes `nonce N` first.
  The submitted tx now reverts with `WrongNonce` — relayer pays the rbb/intrinsic gas of a reverting tx.
...
`(len|len|len|rbb|exp|mod)` layout, output length is asserted == 256 (line 199), and `staticcall` failure
...
RBB-mainnet v1.3 deployment**, attacking a **real twin holding real funds**:

- **Target twin:** [`0xcBeaF766D4a7DD61558d4E80ee58B8B8379d4CEf`](https://rbbscan.org/address/0xcBeaF766D4a7DD61558d4E80ee58B8B8379d4CEf) — Twitch `yougotcoined` (`userId 1507305235`), balance ~`0.0002 ETH`, state at test time: `nonce 0`, `ownerEOA 0x0`, `activated false`, `selfCustody false`.
