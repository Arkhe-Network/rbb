...
Everything needed to (re)verify the SocialTwin contracts on RBBscan, including
...
- **Twins** (`TwinAccount`) are deployed by the factory via CREATE2, so RBBscan
...
## Live addresses — v1.3 (RBB mainnet · chainId 12120014)

| Contract | Address | RBBscan |
|---|---|---|
| `TwinFactory` | `0x260C074c3afDc46A209D4619B5FAdB2964dF9a28` | [code](https://rbbscan.org/address/0x260C074c3afDc46A209D4619B5FAdB2964dF9a28#code) |
| `TwitchJWTVerifier` | `0xBDfC552469f11843802BCD7ec9a8372c8020fee8` | [code](https://rbbscan.org/address/0xBDfC552469f11843802BCD7ec9a8372c8020fee8#code) |
| `TwinAccount` (reference twin, `userId 1507305235`) | `0xcBeaF766D4a7DD61558d4E80ee58B8B8379d4CEf` | [code](https://rbbscan.org/address/0xcBeaF766D4a7DD61558d4E80ee58B8B8379d4CEf#code) |
...
`RBB_API_KEY` (and `RBB_RPC_URL`, `PRIVATE_KEY`) live in `../zkx402/.env`,
...
grep -E '^(RBB_API_KEY|RBB_RPC_URL)=' ../zkx402/.env > .env
# ... run the verify command ...
rm -f .env
```

Verification needs only `RBB_API_KEY` (no private key — verify sends no tx).
...
npx hardhat verify --network rbb \
...
contract with `--contract` (RBBscan can't pick `TwinAccount` from bytecode alone
...
const p = new ethers.JsonRpcProvider("https://mainnet.rbb.org");
...
Once one `TwinAccount` is verified, RBBscan auto-flags later twins as
...
which RBBscan masks during matching — so propagation **usually** works within
minutes. Caveats:

- Not contractually guaranteed by RBBscan; it can lag or miss.
...
```js
const url = `https://api.etherscan.io/v2/api?chainid=12120014&module=contract` +
...
| Symptom | Cause / fix |
|---|---|
| `Already Verified` | Nothing to do — RBBscan already has matching source. |
...
| Verify hangs on "Waiting for result" | RBBscan indexing lag; the contract must have ≥1 confirmation first. |

---

## External references

- RBBscan: <https://rbbscan.org>
- Etherscan v2 multichain API (chainid 12120014 = RBB): <https://docs.etherscan.io/etherscan-v2>
