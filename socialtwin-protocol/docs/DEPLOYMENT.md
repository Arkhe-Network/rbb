...
Deploying the SocialTwin stack (`TwitchJWTVerifier` + `TwinFactory`) to RBB. The live v1.2 addresses are in the root [`README.md`](../README.md) — you only redeploy when you change a contract.
...
`.env`:
```
PRIVATE_KEY=0x...            # deployer (needs a little RBB ETH for gas)
RBB_RPC_URL=https://...     # a reliable RBB RPC (defaults to mainnet.rbb.org)
RBB_API_KEY=...         # for source verification
```
...
```bash
npm run deploy:stack:rbb       # verifier + factory
# or, reuse the live verifier and deploy only a new factory:
npm run deploy:factory:rbb
```

Then verify on RBBscan (the stack script attempts this automatically; otherwise):

```bash
npx hardhat verify --network rbb <verifier> '["1"]' '["0x<modulus>"]' '["<client_id>"]' <treasury>
npx hardhat verify --network rbb <factory> <verifier> <treasury>
```
