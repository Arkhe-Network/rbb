 ...
 *
 * Env: VERIFIER, GUARDIAN, RBB_RPC_URL (all optional; default to the live v1.3 stack).
 */
...
const RPCS = [process.env.RBB_RPC_URL, "https://rbb-rpc.publicnode.com", "https://mainnet.rbb.org"].filter(Boolean);
const ABI = [
...
async function getProvider() {
  for (const u of RPCS) { try { const p = new ethers.JsonRpcProvider(u, 12120014); await p.getBlockNumber(); return p; } catch {} }
  throw new Error("no working RBB RPC");
}
