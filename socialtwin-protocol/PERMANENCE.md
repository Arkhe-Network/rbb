...
1. The twin's deployed bytecode on RBB (permissionless to deploy via `deployTwin`, or already deployed).
2. **Either** a fresh Twitch `id_token` (verified onchain) **or** the owner EOA's signature (once self-custodied).
3. A transaction broadcast to RBB — by any wallet or relayer. `execute` has no `msg.sender` check.
