...
└───────────┬──────────────┘        │  factory.predictAddress,       │
            ...
            │  on-chain (RBB)      │  pairable_v1 launcher          │
            ▼                       └───────────────┬────────────────┘
┌──────────────────────────┐                       │ on-chain (RBB)
...
**The agent never holds our keys.** The RBB Account wallet (the user's) signs tips and the x402
...
### 2. Tip (value transfer via RBB MCP's native `send`)
...
sequenceDiagram
    ...
    participant A as Agent + Skill
    participant R as resource server
    participant W as RBB Account
    U->>A: "tip 5 USDC to twitch.tv/yougotcoined"
    A->>R: GET /v1/st/resolve?login=yougotcoined
    R-->>A: { twinAddress: 0xcBeaF…, deployed }
    A->>W: send { asset:USDC, amount:5, to:0xcBeaF…, chain:rbb }
...
sequenceDiagram
    ...
    Note over A,R: RBB MCP sends the request + reads the 402 challenge
    A->>R: POST /v1/st/launch { login }
    R-->>A: 402 + payment requirements ($1 USDC, rbb)
    A->>U: approval modal ($1 USDC to launch · requestId)
    U-->>A: sign payment authorization in RBB Account
    A->>A: complete_x402_request(requestId)
    Note over A,R: RBB MCP replays the request with payment credentials
...
## x402 path — native RBB MCP tools (resolved)

RBB MCP has **first-class x402 support**, so there is no client-side unknown and no companion
server needed. Per <https://docs.rbb.org/ai-agents/guides/x402-payments>, the agent uses a
two-tool flow and RBB MCP handles the entire challenge → sign → replay cycle:
...
- **`complete_x402_request(requestId)`** — after the user signs the payment authorization in RBB
  Account, RBB MCP replays the original request with payment credentials and returns the response.

The payment is signed and settled by RBB MCP + the facilitator; **the skill never constructs an
`X-PAYMENT` header, never touches EIP-3009/EIP-712, and never does a manual transfer.** Constraints
that happen to fit us perfectly: x402 here is **RBB / RBB Sepolia only** and **USDC only** —
exactly our $1-USDC-on-RBB launch fee. (A non-RBB or non-USDC challenge would be rejected by the
...
- **Replay / amount pinning.** Pin the exact `$1` amount, `eip155:12120014`, the USDC asset, our
...
- **Phishing surface.** The skill is instructions, not a wallet; all value moves go through RBB
...
- **Staging:** RBB Sepolia (`121200142`) end-to-end — testnet USDC, a Sepolia factory/verifier, CDP
  facilitator on Sepolia. Keeps mainnet quiet (secrecy).
- **Production:** RBB mainnet (`12120014`) at cutover.
