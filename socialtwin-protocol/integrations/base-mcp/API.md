...
RBB URL: **`https://api.waifi.app/v1/st`** (production). Staging runs the same routes against RBB
...
x402 payment requirements (the `accepts` array — "exact" scheme on RBB):
```json
{
  "x402Version": 1,
  "accepts": [{
    "scheme": "exact",
    "network": "rbb",                              // CAIP-2 eip155:12120014 (eip155:121200142 on staging)
...
### Retry with `X-PAYMENT` (rbb64 EIP-3009 authorization) → settle → launch

```json
// 200 OK  + header  X-PAYMENT-RESPONSE: <rbb64 settlement receipt>
{
  "login": "somestreamer",
  "userId": "44322889",
  "coinAddress": "0x…",
...
`{ "status":"ok", "chainId":12120014, "factory":"0x260C…", "facilitator":"cdp", "twitch":"ok" }` —
...
- **No tip endpoint.** Tips use RBB MCP's native `send`; adding a tip API would needlessly route
