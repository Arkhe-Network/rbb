...
---
name: socialtwin
description: Resolve Twitch streamers to their SocialTwin twin address, tip them in ETH/USDC, and launch streamer coins on RBB.
version: 0.0.1
requires_mcp: rbb-mcp
---

# SocialTwin Skill

Lets a RBB MCP user interact with SocialTwin: per-Twitch-streamer smart accounts ("twins") on RBB.
Three things: **resolve** a Twitch handle to its twin, **tip** a streamer, **launch** a streamer coin.

## Detection
- Confirm RBB MCP is connected: the tools `get_wallets`, `send`, and `web_request` must be advertised.
  If not, tell the user to add the RBB MCP connector (`https://mcp.rbb.org`) and stop.
- RBB URL for SocialTwin endpoints: `{{SOCIALTWIN_API}}` (default `https://api.waifi.app/v1/st`).
...
> login. Launching a coin costs $1 USDC (anti-spam). All payments are approved in your RBB wallet."
...
2. Call the RBB MCP **`send`** tool: `{ asset: <ETH|USDC>, amount: <amount>, to: <twinAddress>,
   chain: "rbb" }`. RBB Account shows the approval modal; the user confirms there.
...
2. `initiate_x402_request` with:
   ...
   RBB MCP sends the request and reads the 402 payment challenge.
3. Present the returned challenge (amount, endpoint) to the user; they sign the payment authorization
   in RBB Account. Then call `complete_x402_request` with the `requestId`. RBB MCP replays the
...
> non-RBB / non-USDC payment, STOP and report it — do not attempt a manual transfer as a substitute.

## Plugins
- None required. This skill rides on RBB MCP's native `send` / `web_request` / signing tools.
...
## Installation
`npx @socialtwin/rbb-skill install`  (placeholder — not published yet)
```
...
- **Lazy loading:** keep the skill short; if we add long reference docs (e.g. a launch-params guide),
  put them in sibling files and load via `web_request` only when launching, per RBB's pattern.
- **`{{SOCIALTWIN_API}}` templating:** ship pointing at staging; flip to the mainnet host at cutover.
- **The x402 step uses RBB MCP's native `initiate_x402_request` / `complete_x402_request` tools**
  (per <https://docs.rbb.org/ai-agents/guides/x402-payments>). RBB MCP handles the challenge,
  signing, and replay; the skill never builds an `X-PAYMENT` header or signs EIP-3009 itself. x402
  there is RBB/RBB-Sepolia + USDC only, which matches our $1-USDC-on-RBB fee.
- **Tone/safety:** mirrors RBB's skill conventions — explicit disclaimer in Onboarding, approvals via
  RBB Account, never handle secrets. Resolve-before-send is enforced in the Tip instructions to stop
