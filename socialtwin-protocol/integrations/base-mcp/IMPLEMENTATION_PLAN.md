...
- **Reuse, don't rebuild.** Tips ride RBB MCP's `send`; resolution rides `predictAddress`; launch
...
The original blocker (how RBB MCP pays x402) is **already answered** by the RBB docs: native
`initiate_x402_request` / `complete_x402_request` tools, RBB + USDC only — see
...
- **Q2:** Confirm `send` accepts a bare address `to` (not only ENS) and supports both ETH and USDC on RBB.
- **Q3:** Confirm the CDP facilitator covers RBB Sepolia for staging, and the mainnet free-tier limit (≈1k/mo) is enough.
- **Smoke:** pay a throwaway $0.001 Sepolia x402 endpoint via `initiate_x402_request`/`complete_x402_request` from a real RBB MCP client to confirm the tool round-trip end to end.
...
- **Demo:** "twin for twitch.tv/<handle>?" returns the right address in a RBB MCP client.
...
- **Demo:** tip flows end-to-end with RBB Account approval; streamer claims via Twitch JWT.
...
1. Point endpoints + skill `{{SOCIALTWIN_API}}` at the mainnet host; `network` → `eip155:12120014`; USDC → `0x8335…2913`.
2. Smoke-test resolve/tip/launch on mainnet with a small real amount.
3. Publish the skill (npm / RBB skills registry) and announce. **This is the first public moment.**
...
- **R1 — x402-in-RBB-MCP maturity — RESOLVED.** RBB MCP ships native `initiate_x402_request` /
  `complete_x402_request` (RBB + USDC). No companion server, no manual signing. Was the feared blocker; isn't.
...
- Resolve: correct twin for any valid Twitch handle, in a real RBB MCP client, < 1s p50.
