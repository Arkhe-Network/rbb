...
class ZvecHybridMemory:
    ...
    def __init__(self, collection_name: str = "hybrid_episodic_v3", state_dim: int = 288):
        ...
        try:
            zvec.init()
            schema = zvec.CollectionSchema(name=collection_name, fields=[
                zvec.FieldSchema("id", zvec.DataType.UINT64),
                zvec.FieldSchema("reward", zvec.DataType.FLOAT),
            ], vectors=[
                zvec.VectorSchema("rssm_dense", zvec.DataType.VECTOR_FP32, dimension=state_dim),
                zvec.VectorSchema("yolo_sparse", zvec.DataType.SPARSE_VECTOR_FP32),
                zvec.VectorSchema("action", zvec.DataType.VECTOR_FP32, dimension=1),
            ])
            try:
                self.collection = zvec.open(collection_name)
            except Exception:
    ...
    def store_hybrid(self, rssm_state: np.ndarray, yolo_tags: List[str], action: np.ndarray, reward: float):
        ...
        sparse_vec = {abs(hash(tag.lower())) % (2**32): 1.0 for tag in yolo_tags}

        doc = zvec.Doc(id=str(self.next_id), fields={
            "id": self.next_id,
            "reward": reward
        }, vectors={
            "rssm_dense": rssm_state.tolist(),
            "yolo_sparse": sparse_vec,
            "action": action.tolist() if isinstance(action.tolist(), list) and len(action.tolist()) > 0 else [float(action)] if np.isscalar(action) else [float(action[0])] if len(action) > 0 else [0.0]
        })
    ...
    def hybrid_search(self, query_dense: np.ndarray, query_tags: List[str], top_k: int = 5) -> List[Dict]:
        ...
        if not self.collection: return []
        # zVEC espera índices inteiros (UINT32) para vetores esparsos
        # Mapeando strings para ints usando hash
        query_sparse = {abs(hash(tag.lower())) % (2**32): 1.0 for tag in query_tags}

        # Executa busca híbrida no motor C++ do zVEC
        # Executa busca híbrida usando a API query
        results = self.collection.query(
            queries=[
                zvec.Query(field_name="rssm_dense", vector=query_dense.tolist()),
                zvec.Query(field_name="yolo_sparse", vector=query_sparse)
            ],
            topk=top_k,
            reranker=zvec.RrfReRanker(60)
        )
        return [{"id": r.id, "score": r.score, "action": list(r.vector("action").values()) if isinstance(r.vector("action"), dict) else r.vector("action"), "reward": r.field("reward")} for r in results]
...
class VisionEncoder(nn.Module):
    def __init__(self, device="cpu"):
        super().__init__()
        self.net = nn.Sequential(nn.Conv2d(3, 64, 3, stride=2), nn.ReLU(), nn.AdaptiveAvgPool2d((8, 8)), nn.Flatten(), nn.Linear(64*8*8, 256))
        self.to(device)
    @torch.no_grad()
    ...
    async def cycle(self, frame: np.ndarray) -> Dict:
        ...
        if self.world_model:
            ...
            if self.use_rust_search and self.rust_bridge:
                ...
                similar_memories = self.memory.hybrid_search(latent_288d, yolo_tags, top_k=5)
            else:
                similar_memories = []
