...
pub struct DaemonSnapshot {
    ...
    pub requests_by_opcode: Vec<(u8, u64)>,
...
impl MetricsRing {
    ...
    pub fn snapshot(&self, ring: &[Option<RingEntry>; RING_ENTRIES]) -> DaemonSnapshot {
        ...
        let mut by_opcode_map = HashMap::new();
        for m in &self.entries { *by_opcode_map.entry(m.opcode).or_insert(0u64) += 1; }
        let by_opcode = by_opcode_map.into_iter().collect();
...
pub struct CudaKernelCache {
    ...
    pub fold: CudaFunction,
    pub spielman_encode: CudaFunction,
    pub merkle_level: CudaFunction,
    pub batch_merkle_openings: CudaFunction,
    pub batch_inner_product: CudaFunction,
...
impl CudaKernelCache {
    pub fn new() -> Result<Arc<Self>, Box<dyn std::error::Error>> {
        ...
        // This is a rough estimation of memory
        let total_memory = 32_000_000_000;
        ...
        Ok(Arc::new(Self {
            ...
            stream, device, device_name, total_memory,
        }))
    }
}
    ...
    pub fn get_status(&self) -> DaemonStatus {
        let (cuda_available, cuda_version, device_name, device_memory) =
            if let Some(ref k) = self.kernels {
                (true, "12.2".to_string(), k.device_name.clone(), (k.total_memory / (1024 * 1024)) as u64)
            } else {
...
fn handle_inner_product(acc: &TensorZkpAccelerator, payload: &[u8]) -> (Vec<u8>, u64) {
    ...
    let (result, _gpu_dt) = if let Some(ref kernels) = acc.kernels {
        ...
        let d_a = kernels.device.htod_sync_copy(&even).unwrap();
        let d_b = kernels.device.htod_sync_copy(&odd).unwrap();
        let mut d_out = kernels.device.alloc_zeros::<u32>(grid as usize).unwrap();
...
fn handle_fold(acc: &TensorZkpAccelerator, payload: &[u8]) -> (Vec<u8>, u64) {
    ...
    let (result, _gpu_dt) = if let Some(ref kernels) = acc.kernels {
        let block = 256; let grid = (n as u32 + block - 1) / block;
        let d_in = kernels.device.htod_sync_copy(&odd).unwrap();
...
fn handle_merkle_root(acc: &TensorZkpAccelerator, payload: &[u8]) -> (Vec<u8>, u64) {
    ...
    let (result, _gpu_dt) = if let Some(ref kernels) = acc.kernels {
        ...
        let gpu_t0 = Instant::now();
        while current.len() > 1 {
            let n_cur = current.len() as u32;
            let d_in = kernels.device.htod_sync_copy(&current).unwrap();
            let mut d_out = kernels.device.alloc_zeros::<u32>((n_cur / 2) as usize).unwrap();
...
impl TensorZkpGpuDaemon {
    pub fn new(port_path: &str, baud: u32, metrics_path: PathBuf, gpu_enabled: bool) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self { accelerator, serial, signal, _start_time: Instant::now() })
    }
