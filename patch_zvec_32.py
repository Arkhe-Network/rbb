...
for i, line in enumerate(lines):
    if "            \"action\": action.tolist() if isinstance(action.tolist(), list) else [float(action)]\n" in line:
        new_lines.append("            \"action\": action.tolist() if isinstance(action.tolist(), list) and len(action.tolist()) > 0 else [float(action)] if np.isscalar(action) else [float(action[0])] if len(action) > 0 else [0.0]\n")
        continue
