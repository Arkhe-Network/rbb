...
use alloc::vec::Vec;
use alloc::boxed::Box; // APENAS para debug/logging em builds com alloc; não usado em hot path
...
struct HoneypotState {
    ...
    _interaction_count: u32,
}

impl Default for ProActBFT {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ProActBFT {
    ...
    pub fn silent_quarantine(&mut self, region: RegionId, agent: AgentId, _now_ms: u64) {
        let entry = self.quarantine.entry(region).or_default();
...
impl Default for TemporalSemanticFirewall {
    fn default() -> Self {
        Self::new()
    }
}
...
impl TemporalSemanticFirewall {
    ...
    pub fn tick_reset_circuit_breaker(&mut self, now_ms: u64) {
        if self.circuit_breaker.state() == CircuitState::Open
            && now_ms.saturating_sub(self.circuit_breaker.last_failure_time_ms)
