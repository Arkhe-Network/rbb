// Substrato 989.y.8 - WormGraph Benchmarks v5.2.0
// Criterion benchmarks para performance O(1) e throughput
...
use wormgraph_989y::wormgraph_core::{WormGraph, NodeMetadata, FoundingFather};
...
fn benchmark_semantic_query(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &_size| {
            b.iter(|| {
                let _ = black_box(wg.semantic_query(&query, 10, 0.8));
            });
        });
    }
    group.finish();
...
fn benchmark_zk_nullifier(c: &mut Criterion) {
    c.bench_function("zk_nullifier", |b| {
        ...
        b.iter(|| {
            let _ = black_box(wg.generate_zk_nullifier(&node_id, "private-query"));
        });
    });
}
