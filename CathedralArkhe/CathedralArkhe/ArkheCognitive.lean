...
theorem explainable_uniform (beliefs : Array ℝ) (h_nonempty : 0 < beliefs.size)
  ...
  have h_all_eq_m : ∀ x ∈ beliefs, x = m := by
    ...
    have h_sum : (beliefs.foldl (· + ·) 0) = beliefs.size * beliefs[0] := by
      -- the induction wasn't valid, let's use Array API instead
      have h_size : beliefs.size = beliefs.size := rfl
      sorry
    rw [h_sum]
...
partial def s_measure (loop : ReentryLoop) (initial : AgentState) : ℝ :=
